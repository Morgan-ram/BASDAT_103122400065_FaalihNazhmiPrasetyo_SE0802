...
SELECT last_name, salary, department_id -- Pilih kolom last_name, salary, department_id
FROM employees                          -- Dari tabel employees
WHERE salary IN                         -- Filter: salary harus ada di dalam...
    (SELECT MIN(salary)                 -- ...hasil subquery: ambil salary terkecil...
    FROM employees                      -- ...dari tabel employees...
    GROUP BY department_id);            -- ...per department_id
